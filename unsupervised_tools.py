...
def biplot(
    ...
    for i in range(n):
        plt.arrow(
            0,
            0,
            np.abs(scale_xs).max() * coeff[i, 0],
            np.abs(scale_ys).max() * coeff[i, 1],
            color="r",
        )
        if labels is None:
            plt.text(
                np.abs(scale_xs).max() * coeff[i, 0] * 1.15,
                np.abs(scale_ys).max() * coeff[i, 1] * 1.15,
                "Var" + str(i + 1),
                color="g",
                ha="center",
                va="center",
...
def plot_clusters(
    ...
    if type(df) is not pd.core.frame.DataFrame:
        raise TypeError(
            f"df argument must be a pandas DataFrame, object passed is  {type(df)}"
        )
    if df.shape[1] == 1:
        # Plot silhouette
    ...
    ax1.set_title(
        f"Silhouette plot. Mean silhouette: {round(sample_silhouette_values.mean(),3)}"
    )
    ...
    if df.shape[1] > 2 and show_curves:
        ...
        for i in range(n_clusters):
            ...
            color = cm.nipy_spectral(float(i) / n_clusters)
            df.iloc[samples_cluster, :].T.plot(
                legend=None, color=color, ax=ax3, alpha=alpha_curves
            )
