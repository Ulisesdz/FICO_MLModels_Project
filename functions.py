...
def classification_report(y_true, y_probs, positive_label, threshold=0.5, n_bins=10):
    ...
    return evaluate_classification_metrics(
        y_true, (y_probs > threshold).astype(int), positive_label, auc=auc
    )
...
def cross_validation_decisionTrees(model, X, y, nFolds):
    ...
    for i in range(nFolds):
        ...
        # Calculo la accuracy
        metricas = evaluate_classification_metrics(
            y_valid, (np.array(y_pred) > 0.5).astype(int), 1, auc=None
        )
...
def escribir_resultados_en_csv(nombre_archivo, nombre_modelo, metricas):
    ...
    columnas = ["Modelo", "TP", "FP", "FN", "TN"] + [
        col
        for col in df_modelo.columns
        if col not in ["Modelo", "TP", "FP", "FN", "TN"]
    ]
    ...
    # Si existe, lo abro y lo modifico
        ...
        if nombre_modelo in df_existente["Modelo"].values:
            df_existente = df_existente[
                df_existente["Modelo"] != nombre_modelo
            ]  # Elimino el modelo existente
            df_existente = pd.concat(
                [df_existente, df_modelo], ignore_index=True
            )  # Añado el nuevo modelo
...
def calcular_importancia_variables(modelo, X, y):
    ...
    y_pred_proba_inicial = modelo.predict_proba(X)
    metricas = evaluate_classification_metrics(
        y, (np.array(y_pred_proba_inicial) > 0.5).astype(int), 1, auc=None
    )
    ...
    for i in range(X.shape[1]):
        ...
        y_pred_proba_sin_variable = modelo.predict_proba(X_sin_variable)
        metricas = evaluate_classification_metrics(
            y, (np.array(y_pred_proba_sin_variable) > 0.5).astype(int), 1, auc=None
        )
...
def graficar_importancia_variables(importancia_variables, nombres_variables):
    ...
    importancia_variables_ordenadas, nombres_variables_ordenados = zip(
        *sorted(zip(importancia_absoluta, nombres_variables), reverse=True)
    )

    fig, ax = plt.subplots(figsize=(10, 6))
    ax.bar(
        nombres_variables_ordenados, importancia_variables_ordenadas, color="skyblue"
    )
