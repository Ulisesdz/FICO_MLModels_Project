...
if __name__ == "__main__":
    ...
    contributions = np.abs(pca.components_) * np.sqrt(
        pca.explained_variance_.reshape(-1, 1)
    )
    # Creo un dataframe con la informacion
    ...
    df_contributions = pd.DataFrame(
        contributions.T, index=variable_names, columns=component_names
    )
    # Lo grafico
    plt.figure(figsize=(10, 8))
    sns.heatmap(
        df_contributions, cmap="coolwarm", annot=True, fmt=".2f", linewidths=0.5
    )
    plt.xlabel("Componente Principal")
    plt.ylabel("Variable")
    plt.title(
        "Contribución de cada variable en cada componente principal (Varianza explicada)"
    )
    plt.show()
    ...
    df_proyectados = pd.DataFrame(
        datos_proyectados,
        columns=["Componente Principal " + str(i) for i in range(1, 7)],
    )
    ...
    fig, axs = plt.subplots(
        1, 3, figsize=(15, 5)
    )  # Cambio a una sola fila con tres subplots
    ...
    for n_clusters in range_n_clusters:
        ...
        if plot:
            UT.plot_clusters(
                df, cluster_labels, centers=clusterer.cluster_centers_, figsize=(6, 4)
            )
    ...
    for n_clusters in range_n_clusters:
        clusterer = GaussianMixture(
            n_components=n_clusters, covariance_type=cv_type, random_state=10
        )
        cluster_labels = clusterer.fit_predict(X_transformed)
        ...
        if plot:
            UT.plot_clusters(
                df, cluster_labels, centers=clusterer.means_, figsize=(6, 4)
            )
    ...
    clusterer_GMM = GaussianMixture(
        n_components=2, covariance_type="full", random_state=10
    )
    ...
    UT.plot_clusters(
        df,
        cluster_GMM,
        centers=scaler.inverse_transform(clusterer_GMM.means_),
        figsize=(10, 6),
    )
    ...
    plt.scatter(
        X_pca[y == 0, 0], X_pca[y == 0, 1], color="red", label="RiskPerformance: 0"
    )
    plt.scatter(
        X_pca[y == 1, 0], X_pca[y == 1, 1], color="blue", label="RiskPerformance: 1"
    )
    ...
    axs[1].scatter(
        X_pca[cluster_knn == 0, 0],
        X_pca[cluster_knn == 0, 1],
        color="green",
        label="Cluster 1",
    )
    axs[1].scatter(
        X_pca[cluster_knn == 1, 0],
        X_pca[cluster_knn == 1, 1],
        color="orange",
        label="Cluster 2",
    )
