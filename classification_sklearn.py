...
if __name__ == "__main__":
    ...
    nFolds = 5
    Linear_SVC_gcv = GridSearchCV(
        estimator=pipe,  # Estructura definida
    ...
    fun.escribir_resultados_en_csv("Datasets/MetricasModelos.csv", "SVM Lineal", metricas)
    print("=" * 50)

    # SVM KERNEL Radial
    print("Modelo SVM - Kernel RBF")
    param = {
        "SVC__C": [1000],  # Parametro de coste
        "SVC__gamma": [0.001],
    }  # Inverse width parameter in the Gaussian Radial Basis kernel
    pipe = Pipeline(
        steps=[
            ("scaler", StandardScaler()),
            (
    ...
    metricas = fun.classification_report(y_test, y_pred[:, 1], 1)
    # Guardo los datos en el csv
    fun.escribir_resultados_en_csv("Datasets/MetricasModelos.csv", "SVM Radial", metricas)
    print("=" * 50)
    ...
    metricas = fun.classification_report(y_test, y_pred[:, 1], 1)
    # Guardo los datos en el csv
    fun.escribir_resultados_en_csv("Datasets/MetricasModelos.csv", "NaiveBayes", metricas)
    print("=" * 50)
