...
if __name__ == "__main__":
    ...
    plt.show()
    plt.savefig('Datasets/Histogramas_Variables.png')
    ...
    plt.show()
    plt.savefig('Datasets/Matriz_Correlacion.png')
    ...
    plt.show()
    plt.savefig('Datasets/Distribucion_Output.png')

    # Relaciones entre variables
    sns.pairplot(df, hue="RiskPerformance", diag_kind="kde")
    plt.show()
    plt.savefig('Datasets/Relaciones_Variables.png')
    ...
    X_train, X_test, y_train, y_test = fun.train_test_split(input, output, test_size=0.2, stratify=output, random_state=42)
    ...
    X_train_df = pd.DataFrame(X_train, columns=["ExternalRiskEstimate", "NetFractionRevolvingBurden", "AverageMInFile", "MSinceOldestTradeOpen", "PercentTradesWBalance", "PercentInstallTrades", "NumSatisfactoryTrades", "NumTotalTrades", "PercentTradesNeverDelq", "MSinceMostRecentInqexcl7days"])
    X_test_df = pd.DataFrame(X_test, columns=["ExternalRiskEstimate", "NetFractionRevolvingBurden", "AverageMInFile", "MSinceOldestTradeOpen", "PercentTradesWBalance", "PercentInstallTrades", "NumSatisfactoryTrades", "NumTotalTrades", "PercentTradesNeverDelq", "MSinceMostRecentInqexcl7days"])
