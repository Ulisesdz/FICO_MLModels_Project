...
if __name__ == "__main__":
    ...
    fun.escribir_resultados_en_csv("Datasets/MetricasModelos.csv", "KNN", metricas)
    print("=" * 50)
    ...
    # Guardo los datos en el csv
    fun.escribir_resultados_en_csv("Datasets/MetricasModelos.csv", "LinearRegression", metricas)
    print("=" * 50)
    ...
    optimum_C_values = [
        C for C, acc in zip(C_values, accuracies) if acc == max_accuracy
    ]
    ...
    LogReg_optimum_C_ElasticNet.fit(
        X_train, y_train, penalty="elasticnet", C=optimum_C, verbose=False
    )
    # Obtengo las métricas de cómo ha funcionado en el test
    y_test_prob = LogReg_optimum_C_ElasticNet.predict_proba(X_test)
    metricas = fun.classification_report(y_test, y_test_prob, 1)
    # Guardo los datos en el csv
    fun.escribir_resultados_en_csv(
        "Datasets/MetricasModelos.csv", "LogisticRegression", metricas
    )
    print("=" * 50)
    ...
    max_depth = 6
    # Creo el modelo y lo entreno
    decision_tree = clsf_modl.DecisionTree(
        min_samples=min_num_samples, max_depth=max_depth
    )
    ...
    # Guardo los datos en el csv
    fun.escribir_resultados_en_csv("Datasets/MetricasModelos.csv", "DecisionTree", metricas)
    print("=" * 50)

    # Bagging
    print("Bagging Model")
    # Creo el modelo y lo entreno
    bagging_model = clsf_modl.Bagging(
        n_estimators=100, min_num_samples=min_num_samples, max_depth=max_depth
    )
    ...
    metricas = fun.classification_report(y_test, np.array(y_test_prob), 1)
    # Guardo los datos en el csv
    fun.escribir_resultados_en_csv("Datasets/MetricasModelos.csv", "Bagging", metricas)
    print("=" * 50)

    # Random Forest
    print("Random Forest Model")
    # Creo el modelo y lo entreno
    random_forest = clsf_modl.RandomForest(
        n_estimators=100, min_num_samples=min_num_samples, max_depth=max_depth
    )
    ...
    metricas = fun.classification_report(y_test, np.array(y_test_prob), 1)
    # Guardo los datos en el csv
    fun.escribir_resultados_en_csv("Datasets/MetricasModelos.csv", "RandomForest", metricas)
    print("=" * 50)
