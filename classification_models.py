...
class knn:
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray, k: int = 5, p: int = 2):
        ...
        if X_train.shape[0] != y_train.shape[0]:
            raise ValueError(
                f"Los X_train y y_train datasets no tienen el mismo tamaño"
            )
        self.x_train = X_train
        self.y_train = y_train
        for numero in [k, p]:
        ...
        for data_point in self.x_train:
            if not np.array_equal(data_point, point):
                ...
                if distancia is None:
                    print(
                        f"La distancia es None para data_point: {data_point}, point: {point}"
                    )
...
class LogisticRegressor:
    ...
    def fit(
        self, X, y, penalty=None, l1_ratio=0.5, C=1.0, verbose=False, print_every=100
    ):
        """
    ...
    def elasticnet_regularization(self, dw, m, C, l1_ratio):
        ...
        elasticnet_gradient = (
            l1_ratio * lasso_gradient + (1 - l1_ratio) * ridge_gradient
        )
    ...
    def find_best_split(self, X, y):
        ...
        for feature_index in range(X.shape[1]):
            ...
            for threshold in thresholds:
                left_X, left_y, right_X, right_y = self.split_data(
...
class Bagging:
    def __init__(
        self, n_estimators=100, max_samples=0.8, max_depth=None, min_num_samples=None
    ):
        """
    ...
    def fit(self, X_train, y_train):
        ...
        for _ in range(self.n_estimators):
            ...
            sample_indices = np.random.choice(
                range(n_samples), size=int(n_samples * self.max_samples), replace=True
            )
            ...
            decision_tree = DecisionTree(
                max_depth=self.max_depth, min_samples=self.min_num_samples
            )
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_estimators):
            ...
            selected_features = np.random.choice(
                range(n_features), size=int(n_selected_features), replace=False
            )
